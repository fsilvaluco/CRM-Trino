...
**Crear un MCP dedicado para ArtistPro (Supabase, con escritura)** _(agregado: 10 sep 2026, idea a madurar)_
- Contexto: Francisco está armando un agente en **Munder Difflin** (su "agent harness" corriendo en una VM aparte, ver [munderdiffl.in](https://munderdiffl.in)) para revisar tareas pendientes de varios proyectos (Trino, Katarsis, SiSoy, Los Últimos Románticos, Gamuza, Simplemente Yo) y mandar un resumen diario en texto para WhatsApp.
- **Fase 1 (en curso, resuelta con lo que ya existe):** el agente de solo-lectura usa el MCP oficial de Supabase (`@supabase/mcp-server-supabase`, modo `--read-only`) apuntando al proyecto `nfvzflgxnfhqbfrfxhjl` — no requirió código nuevo.
- **Pendiente (esto):** `mcp/crm-server.ts` (el MCP propio del proyecto, mencionado en `CLAUDE.md`) está **obsoleto** — quedó armado para el modo viejo SQLite local (`better-sqlite3` contra `data/crm.db`), de antes de migrar a Supabase, y ni siquiera expone Tareas. Si más adelante se quiere que un agente **actualice** tareas/agregue comentarios (ej. a partir de respuestas en un grupo de WhatsApp: "ya hice esto", "está en la carpeta X"), no conviene que escriba directo a la base por SQL — se saltaría los chequeos de permisos por rol/proyecto, las push notifications y la detección de menciones @ que sí aplican `PUT /api/tasks/[id]` y `POST /api/tasks/[id]/comments`. La opción correcta sería reescribir `mcp/crm-server.ts` (o un MCP nuevo) para que hable con Supabase y llame al API real de la app autenticado como un usuario "bot" dedicado, en vez de tocar las tablas directo. No empezado — falta definir con Francisco el mecanismo de autenticación del bot y cómo se decide a qué tarea corresponde cada respuesta de WhatsApp (texto libre es ambiguo).
...
- **✅ [Bajos/Info] Resueltos (10 sep 2026, trabajo nocturno sin supervisión -- ver rama `chore/security-hardening-low-severity`):**
  - **Comparación timing-safe del secreto de cron**: los 5 endpoints `/api/cron/*` comparaban el header `Authorization` con `!==` (string comparison normal). Extraído a `src/lib/cron-auth.ts` (`verifyCronSecret()`, usa `crypto.timingSafeEqual`), mismo contrato/mensajes.
  - **Mensajes de error de Postgres sanitizados**: `org-members` (GET/POST/PATCH/DELETE, ~12 call-sites) y `auth/activate` devolvían `error.message` crudo en las respuestas 500. Nuevo helper `src/lib/api-errors.ts` (`dbErrorResponse()`) -- loguea el error completo server-side, devuelve mensaje genérico al cliente.
  - **Dockerfile**: corre como usuario `node` (no root) para el proceso final; nuevo `HEALTHCHECK` contra `/api/health` (ruta nueva, pública, sin auth, no toca la DB a propósito); `docker-compose.yml` con límites de recursos documentados (comentados -- sin conocer el servidor real, mejor que Francisco los ajuste). **No probado con `docker build` real** (Docker no estaba disponible en el entorno donde se hizo el cambio) -- falta esa prueba antes de confiar en esto a ciegas.
  - **Dependencias fijadas a versión exacta** (sin `^`) en `package.json` -- ningún paquete cambió de versión, solo se dejó de permitir que cambie solo en un `npm install` futuro.
  - Además, de paso: se excluyeron del lint los 3 scripts de debug (`test-*.js/ts` de la raíz) en vez de reescribirlos (son CommonJS de un solo uso, no parte de la app).
  - **Verificado en cada commit**: `npx tsc --noEmit`, `eslint` (archivos tocados) y `npm run build`, todos limpios.
- **🔴 Encontrado de paso (10 sep 2026, sin resolver -- decide Francisco):** `test-with-login.js` (script de debug en la raíz) tiene una lista de contraseñas comunes hardcodeadas que prueba contra emails reales de Francisco (`francisco@agenciakatarsis.cl` y otros). No es código malicioso -- es un script propio para testear login a mano -- pero tenerlo commiteado en el repo con emails reales + intentos de contraseña no es buena práctica. Decidir: borrarlo, o al menos dejar de commitearlo.
- **🟡 Encontrado de paso, no resuelto:** `npm audit` marca 14 vulnerabilidades en dependencias transitivas (1 baja, 9 moderadas, 3 altas, 1 crítica) -- no se tocó esta noche, `npm audit fix --force` puede traer breaking changes, requiere revisar caso a caso qué paquete es y si de verdad aplica a como se usa acá.
- **🟡 Encontrado de paso, no resuelto:** `eslint .` sobre todo el repo marca **1496 problemas** (27 errores, 1469 warnings) fuera de los 3 archivos de debug -- mucho más que los "~50" que se creía en la entrada del 23 ago. La mayoría son warnings de estilo (hooks sin dependencia declarada, imports sin usar, `<img>` sin optimizar), pero hay **2 errores reales** dignos de mirar pronto: `CommentsWithMentions.tsx:77` y `notifications-context.tsx:35` llaman `setState` directo dentro de un `useEffect` (React lo señala como causa de renders en cascada). No se tocó esta noche -- son componentes de producción, cualquier cambio ahí necesita probarse en el navegador con un humano presente, no a ciegas de madrugada.
