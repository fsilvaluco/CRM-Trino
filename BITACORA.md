...
- **✅ Bug de producción encontrado y corregido (18 ago 2026):** Francisco reportó que al adjuntar el comprobante de pago y apretar "Guardar costos" el dato no quedaba -- desaparecía al recargar (probado 3 veces en el evento "Gamuza: otra noche más aquí en Plaza Victoria 1", pago al sonidista). **Causa raíz:** `saveCosts()` arma su propio payload a mano (no reusa el objeto `CostItem` completo) y se le olvidó incluir `category`, `pagado` y `comprobantePagoUrl` en el `map()` que arma el body del PUT -- el checkbox y el archivo sí se subían y quedaban en el estado local (por eso se veían tildados en pantalla antes de guardar), pero el guardado real a la base nunca los mandaba, así que quedaban descartados en cuanto se recargaba la página. Esto también afectaba silenciosamente a **categoría** (no se había notado). Corregido agregando los 3 campos al payload.
- **Cuadro "Link para gastos" sacado de la Planilla** (pedido explícito de Francisco, 18 ago 2026): quedaba visualmente cargado: el botón se sacó de los controles de Costos para dejarlo más limpio -- la página `/eventos/[id]/gastos` sigue funcionando igual si se comparte el link manualmente, solo ya no hay un botón dedicado ahí.
