...
**Pendientes de la auditoría de seguridad `/cyber-neo` (23 ago 2026)** — reporte completo en `~/Desktop/cyber-neo-report-CRM-Trino-2026-08-23.md`. Se corrigió CN-001 (ver arriba, sección 🔴 Crítico); el resto queda pendiente:
- **[CN-002 · Alto]** Los tokens de acceso de Meta/Instagram (incluyendo tokens de página) quedan logueados en texto plano en `src/app/api/integrations/meta/callback/route.ts` (`console.log`/`console.error` del body completo de la respuesta de Meta). Cualquiera con acceso a los logs del servidor podría tomar esos tokens. Hay que redactar el `access_token` antes de loguear, y considerar rotar el secreto de la app de Meta si esos logs ya se persistieron en algún lado.
- **[CN-003 · Alto]** La dependencia `xlsx` (0.18.5) tiene 2 CVEs (contaminación de prototipo + ReDoS) sin fix publicado en npm (SheetJS dejó de publicar ahí). Hay que fijarla al tarball del CDN oficial (`cdn.sheetjs.com`) o migrar a `exceljs`.
- **[CN-004 · Alto]** Next.js está en 16.2.2, con ~20 vulnerabilidades conocidas (SSRF, bypass de middleware, DoS) ya corregidas en 16.3.2. Es un upgrade no-mayor: `npm install next@16.3.2`.
- **[CN-005 · Medio]** SSRF en los endpoints de extracción de prensa/tickets (`analytics/press-extract`, `eventos/tickets-extract`) — aceptan una URL del usuario y hacen `fetch()` sin bloquear IPs privadas/internas (ej. metadata de nube). Falta un allowlist de hosts o bloqueo de rangos privados.
- **[CN-006 · Medio]** Faltan cabeceras de seguridad HTTP globales (`X-Content-Type-Options`, `X-Frame-Options`, `HSTS`, `Referrer-Policy`) en `next.config.ts` -- solo están configuradas para `/embed/:path*`.
- **[CN-007 · Medio]** El archivo interno de SQLite `data/crm.db-shm` quedó versionado en git por un hueco en `.gitignore` (excluye `.db`/`.db-wal`/`.db-journal` pero no `.db-shm`). Hay que agregarlo al `.gitignore` y sacarlo del tracking (`git rm --cached`).
