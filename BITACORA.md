...
**"Caja de financiamiento" para el LP de Los Últimos Románticos -- se resolvió reusando Finanzas** _(agregado: 19 ago 2026)_
- Francisco trajo un presupuesto real (CSV, 40 ítems) del LP de "Los Últimos Románticos" -- financiado con $3.000.000 en préstamos de familiares/empresas de los 2 artistas (Nacho: $1M empresa + $1M tío, ya entregados a Francisco; Simplemente Yo: $1M de su tío, **todavía no entregado**), que los artistas le van devolviendo en cuotas mensuales para que Francisco se las reparta a los prestamistas. Pidió una "caja": ítems a pagar con comprobante, y los ingresos/egresos de las cuotas.
- **Decisión: no se construyó un módulo nuevo** -- el módulo **Finanzas** que ya existía (tabla `transactions`, ya filtrable por proyecto, con monto/categoría/responsable/comprobante/`reimbursed` y un Balance = Ingresos − Egresos calculado solo) ya cubre exactamente esto. El proyecto **"Los Últimos Románticos"** ya existía en el sistema. Se usó tal cual, filtrando Finanzas por ese proyecto.
- Único cambio de código: se agregaron categorías nuevas a `TransactionForm.tsx` para que calzaran con el rubro de producción musical -- **Masterización, Mezcla, Grabación, Prensa/PR, Video, Fotografía, Campaña ADS, Pago de préstamo** (gastos) y **Préstamo** (ingresos). Antes solo estaban las genéricas de evento (Transporte, Equipamiento, etc.), pensadas para Costos de eventos en vivo, no para presupuestos de grabación/lanzamiento.
- **Se importaron los 40 ítems del CSV** como transacciones `expense` del proyecto LUR (categoría según la columna "Ítem" del CSV, `reimbursed = true` para los marcados "Pagado", comprobante todavía sin adjuntar -- eso lo sube Francisco cuando los tenga a mano). Se excluyó 1 fila con monto $0 (Drip/Fuzz). La columna "Quién paga?" del CSV (qué préstamo financia esa línea) no tiene campo propio en `transactions`, así que se agregó como texto dentro de la descripción (ej. "Dopamina · Financia: SoloNacho").
- **Se registraron los $2.000.000 ya recibidos** como 2 transacciones `income` separadas (categoría Préstamo, responsable "Nacho (empresa)" / "Nacho (tío)") -- el millón de Simplemente Yo **no se registró todavía** porque Francisco dijo explícitamente que aún no se lo entregan; agregarlo cuando confirme que lo recibió.
- **✅ Bug propio encontrado y corregido (19 ago 2026):** al importar los 40 ítems los monté como centavos (`pesos * 100`), igual que en el resto de la app (Costos de eventos, Deals, etc.) -- pero Francisco vio en pantalla montos 100x más grandes ($6.000.000 en vez de $60.000). **Causa: el módulo Finanzas es la excepción** -- `TransactionForm.tsx`/`api/finances/route.ts` guardan y muestran el monto **en pesos directos, sin multiplicar por 100** (`formatCLP()` no divide por 100, el form no multiplica al guardar). No está documentado en ningún lado, hay que acordarse: **Finanzas = pesos, todo el resto de la app (Eventos/Costos, Deals) = centavos.** Corregido con un `UPDATE ... SET amount = amount / 100` sobre las 42 filas insertadas (verificado que eran exactamente las que yo había creado antes de tocar nada, no había transacciones previas de este proyecto).
- **Verificado con totales reales** (ya en pesos correctos): presupuesto total $4.184.500 (40 ítems), de los cuales $645.000 ya pagados. Balance del proyecto queda negativo por ahora (-$2.184.500) porque la mayoría de los ítems siguen "Por pagar" y todavía falta por recibir el tercer préstamo -- **avisarle a Francisco que es normal, no es un error**.
- **Cómo sigue usándolo Francisco de acá en adelante** (no requiere más desarrollo): cada mes, registrar 2 ingresos (categoría Transferencia u otra, ~$150.000 cada uno, uno por artista) y los egresos correspondientes a cada prestamista (categoría "Pago de préstamo", ~$85.000 cada uno) directo desde `/finances` con el proyecto LUR seleccionado. Marcar "Pagado" (reimbursed) y subir comprobante en cada ítem del presupuesto a medida que se van pagando.
- **Pendiente a futuro, no pedido todavía**: si más adelante Francisco quiere ver "cuánto le queda debiendo a cada prestamista" como un saldo corriente (no solo la lista de movimientos), eso requeriría una vista nueva -- no se construyó porque no lo pidió, Finanzas ya le da el registro completo para calcularlo a mano o pedirlo después.
